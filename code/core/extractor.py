...
class Extractor:
    ...
    def extract(self, origin_sentence, sentence, file_path, num):
        ...
        Returns:
            num： 知识三元组的数量编号
        """
        self.get_entities(sentence)    ####调用后面的get_entities函数，得到一个实体列表entities
        self.get_entity_pairs(sentence)###调用后面的get_entity_pairs函数，得到一个实体对列表entity_pairs，每个元素为实体对entity_pair
    ...
    def get_entities(self, sentence):
        ...
        for word in sentence.words:
            if self.is_entity(word):
                self.entities.append(word)   ###得到entities实体列表
